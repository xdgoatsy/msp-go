...
MathStudyPlatform 是一个高等数学学习平台。当前后端迁移版以 Go 服务作为唯一默认运行入口，承接认证、学习、题库、班级、资源、上传、西电教务、管理员设置、统计和安全审计等非 AI 能力；旧 Python FastAPI 后端仅保留为历史参考和迁移对照。
...
2. **接口边界保留**
   ...
   - 后续恢复 AI 能力时应通过模块化接口接入新的工作流，而不是回退到 legacy Python 服务。
...
| **数据库迁移** | Go migration runner + legacy Alembic baseline | Go 后端 |
| **数据库** | PostgreSQL + pgvector | 18+ |
| **缓存** | Redis | 7+ |
| **认证** | Go JWT + Cookie 兼容层 | Go 后端 |
| **AI 工作流** | TODO，占位接口；legacy LangGraph 仅作参考 | P6 待设计 |
| **LLM 集成** | TODO，占位接口；legacy LiteLLM 仅作参考 | P6 待设计 |
| **数学计算** | TODO；legacy SymPy 仅作参考 | P6 待设计 |
| **图像/OCR** | TODO；legacy Pillow/pytesseract 仅作参考 | P6 待设计 |
...
│   └── Dockerfile
│
├── backend/                           # Legacy FastAPI 后端（历史参考，不再默认启动/承流）
│   ├── app/
│   │   ├── main.py                    # 应用入口
│   │   ├── config.py                  # 配置管理
│   │   ├── api/v1/                    # REST API 接口层
...
当前 Go 后端不运行 legacy Python LangGraph/LiteLLM/SymPy/OCR 工作流。旧 Python 目录中的智能体代码只作为历史参考，生产启动、Docker Compose 和 Nginx 默认入口均指向 Go。
...
| 智能刷题 | `/exercise` | BKT 驱动的自适应练习；OCR/LLM 诊断 TODO |
...
- **Node.js** >= 18.x
- **Go** >= 1.25
- **Python** >= 3.11（仅 legacy 后端对照和历史 Alembic 手动操作需要）
...
> 默认生产启动不运行 Python 或 Alembic。Go 后端镜像包含 `msp-migrate`，`scripts/deploy.sh` 和 `scripts/update.sh` 会在启动应用容器前运行 Go migration runner；legacy Alembic 仅保留为历史基线参考。
...
cd backend-go
go test ./...
go test ./tests/contract -count=1   # legacy Python v1 route surface vs Go route surface
```

**Legacy Python 后端（历史参考）**
```bash
cd backend
pytest                        # 运行所有测试
...
# Legacy Alembic 仅用于历史对照，不参与默认启动/部署
```
...
`backend-go/migrations/0001_initial_schema.up.sql` 是从 legacy Alembic head 冻结出的 Go 初始 schema，后续生产迁移由 Go migration runner 承接。`backend/alembic/` 保留为历史参考，不参与默认启动和部署。
...
- 🚧 P8 双跑与契约验证：需要系统化验证 Go 与 legacy Python 的非 AI API 契约差异
- 🚧 P9 Python 下线：默认入口已切 Go，仍需完成归档范围、回滚手册和最终下线记录
